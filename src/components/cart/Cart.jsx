...
export default function Cart({ onDeleteItem, onCartOpen, isCartOpen, gamesInCart}) {
        ...
        <>
            ...
            <div className={`cart ${isCartOpen ? "cart--active" : ""}`}>
                ...
                </div>
                {
                    ...
                    <ul className="items">{gamesInCart.map(game => <CartItem onDeleteItem={onDeleteItem} key={game.id} game={game} />)}</ul>
                    :
...
function CartItem({onDeleteItem, game}) {
    return (
        <li className="item">
        <div className="item__logo"><img className="item__img" src={game.imgUrl} alt="" /></div>
